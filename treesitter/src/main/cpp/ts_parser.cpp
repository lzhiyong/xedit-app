...
void JNICALL parser_delete(JNIEnv *env, jclass clazz, jlong pointer) {
    auto parser = reinterpret_cast<TSParser*>(pointer);
...
void JNICALL parser_reset(JNIEnv *env, jobject thiz) {
    auto parser = GET_POINTER(TSParser, thiz);
    ts_parser_reset(parser);
}

void JNICALL parser_set_language(JNIEnv *env, jobject thiz, jobject value) {    
    auto parser = GET_POINTER(TSParser, thiz);
    auto language = GET_POINTER(TSLanguage, value);
...
void JNICALL parser_set_timeout_micros(JNIEnv *env, jobject thiz, jlong value) {
    auto parser = GET_POINTER(TSParser, thiz);
    ts_parser_set_timeout_micros(parser, static_cast<uint64_t>(value));
...
void JNICALL parser_set_included_ranges(JNIEnv *env, jobject thiz, jobject value) {
    ...
    auto ranges = (TSRange *)calloc(size, sizeof(TSRange));
    ...
    if (ts_parser_set_included_ranges(parser, ranges, size)) {
...
void JNICALL parser_set_cancelled_flag(JNIEnv *env, jobject thiz, jboolean value) {
    auto parser = GET_POINTER(TSParser, thiz);
    ts_parser_set_cancellation_flag(parser, reinterpret_cast<size_t*>(&value));
...
void JNICALL parser_dot_graphs(JNIEnv* env, jobject thiz, jstring pathname) {
    ...
    if(fp < 0) 
        ...
        ts_parser_print_dot_graphs(parser, fp);
...
void JNICALL parser_set_logger(JNIEnv *env, jobject thiz, jobject value) {    
    ...
    TSParser *parser = GET_POINTER(TSParser, thiz);
    TSLogger logger = ts_parser_logger(parser);
    if (logger.payload != nullptr) {
...
jobject JNICALL parser_parse_string(
    ...
    auto language = GET_FIELD(Object, thiz, TSParser_language);
    if (language == nullptr) {
    ...
    auto encoding = static_cast<TSInputEncoding>(
    ...
    // old native TSTree
    auto old_tree = oldTree ? GET_POINTER(TSTree, oldTree) : nullptr;
    // new native TSTree
    auto new_tree = ts_parser_parse_string_encoding(
        parser, old_tree, reinterpret_cast<const char*>(byte_chars), length, encoding
    );
    ...
    if(encoding == TSInputEncodingUTF8) {
        ...
        source = env->NewString(reinterpret_cast<const jchar*>(byte_chars), length);
    }
...
jobject JNICALL parser_parse_function(
    ...
    auto old_tree = oldTree ? GET_POINTER(TSTree, oldTree) : nullptr;
    // new native TSTree
    auto new_tree = ts_parser_parse(parser, old_tree, {(void*)value, callback, encoding});    
    // return the new java TSTree object
