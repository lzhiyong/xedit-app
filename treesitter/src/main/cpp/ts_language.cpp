...
/**
 * Get the TSLanguage address by the grammar function name
 */
jlong JNICALL language_get_grammar(JNIEnv* env, jclass clazz, jstring name) {
...
jboolean JNICALL language_is_named(JNIEnv *env, jobject thiz, jshort symbol) {
    auto language = GET_POINTER(TSLanguage, thiz);
    TSSymbolType symbol_type = ts_language_symbol_type(language, symbol);
...
jstring JNICALL language_field_name_for_id(JNIEnv *env, jobject thiz, jshort id) {
    auto language = GET_POINTER(TSLanguage, thiz);
    const char *name = ts_language_field_name_for_id(language, static_cast<uint16_t>(id));
...
jint JNICALL language_field_id_for_name(JNIEnv *env, jobject thiz, jstring name) {
    ...
    TSFieldId field = ts_language_field_id_for_name(language, field_name, strlen(field_name));   
...
jshort JNICALL language_next_state(JNIEnv *env, jobject thiz, jshort state, jshort symbol) {
    auto language = GET_POINTER(TSLanguage, thiz);
    return ts_language_next_state(language, static_cast<uint16_t>(state), static_cast<uint16_t>(symbol));
}
...
jobject JNICALL language_symbol_type(JNIEnv* env, jobject thiz, jshort symbol) {
    auto language = GET_POINTER(TSLanguage, thiz);
    TSSymbolType symbol_type = ts_language_symbol_type(language, static_cast<uint16_t>(symbol));
...
extern const JNINativeMethod TSLanguage_methods[] = {
    {"getGrammar", "(Ljava/lang/String;)J", (void *)&language_get_grammar},
