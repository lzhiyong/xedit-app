...
class TSQuery @Throws(TSQueryError::class) constructor(
    ...
    // TSQuery pointer
    private val pointer: Long = init(language.pointer, pattern)
    ...
    init {
        RefCleaner(this, CleanAction(pointer, cursor))
    ...
    override fun close() = delete(pointer, cursor)
    
    private class CleanAction(private val pointer: Long, private val cursor: Long) : Runnable {
        override fun run() = delete(pointer, cursor)
    }
