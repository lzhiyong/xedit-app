...
import io.github.module.editor.EditorStack
import io.github.module.editor.util.*
import io.github.module.editor.R

import io.github.module.piecetable.PieceTreeTextBuffer
import io.github.module.piecetable.common.CharCode
import io.github.module.piecetable.common.ContentChange
import io.github.module.piecetable.common.Position
import io.github.module.piecetable.common.Range
import io.github.module.piecetable.common.ReverseEditOperation
import io.github.module.piecetable.common.SingleEditOperation
...
open class EditorView @JvmOverloads constructor(
    ...
    protected val editorStack: EditorStack
    ...
    protected var mergeTextChanges: List<TextChange>? = null
    ...
    protected val cacheRenderNodes = mutableListOf<TextRenderNode>()
    ...
    protected data class TextRenderNode(
        var line: Int = 1,
    ...
    init {
        ...
        cursorBarThumb = context.getDrawable(R.drawable.ic_text_cursor_material)!!.apply {
            setBounds(0, 0, intrinsicWidth, intrinsicHeight)
        }
        
        // the text select handle drawable resources
        selectHandleLeft = context.getDrawable(R.drawable.ic_text_select_handle_left)!!.apply {
            setBounds(0, 0, intrinsicWidth, intrinsicHeight)
        }
        selectHandleRight = context.getDrawable(R.drawable.ic_text_select_handle_right)!!.apply {
            setBounds(0, 0, intrinsicWidth, intrinsicHeight)
        }   
        selectHandleMiddle = context.getDrawable(R.drawable.ic_text_select_handle_middle)!!.apply {
            setBounds(0, 0, intrinsicWidth, intrinsicHeight)
        }
        
        val colorStateList = context.run {
            resources.getColorStateList(R.drawable.scrollbar_state_lists, getTheme())
        }
        
        // vertical scrollbar thumb drawable
        vertScrollbarThumb = context.getDrawable(R.drawable.ic_vert_scrollbar_thumb)!!.apply {
            setBounds(0, 0, intrinsicWidth, intrinsicHeight)
            setTintList(colorStateList)
        }
        
        // horizontal scrollbar thumb drawable
        horizScrollbarThumb = context.getDrawable(R.drawable.ic_horiz_scrollbar_thumb)!!.apply {
            setBounds(0, 0, intrinsicWidth, intrinsicHeight)
            setTintList(colorStateList)
        }
        ...
        pieceTreeBuffer = PieceTreeTextBuffer()
    ...
    fun setText(text: String) {
        val range = Range(1, 1, getLineCount(), getLineEndColumn())
        applyEdits(listOf(SingleEditOperation(range, text)), false)
    }
    ...
    fun setBuffer(pieceBuffer: PieceTreeTextBuffer) {
        pieceTreeBuffer = pieceBuffer
    ...
    fun setEditable(editable: Boolean) {
        cursorBarThumb.setVisible(editable, false)
        /*when(editable) {
            true -> addFocus()
            else -> {
                hideSoftKeyboard()
    ...
    fun setWordwrap(wrap: Boolean) {
        horizScrollbarThumb.setVisible(wrap, false)
        val recycleLayout = textLayout
        textLayout = when(wrap) {
            true -> {
    ...
    fun getLength() = pieceTreeBuffer.getLength()
    ...
    fun getText() = pieceTreeBuffer.getSnapshotContent()
    
    fun getSelectText() = getValueInRange(selection)
    ...
    protected fun getRenderNode(line: Int): TextRenderNode {        
        ...
        // if the node is nonnull meanning which is within the visible range
    ...
    protected open fun drawMatchesBackground(
        ...
        searchMatches.findBy(rLine, startColumn, rLine, endColumn).forEach { range ->       
    ...
    protected open fun drawText(
        canvas: Canvas, text: String, line: Int, start: Int, end: Int, x: Float, y: Float
    ) {
        ...
        reverseEdits?.let { reverses ->
            ...
            */
            ...
            handler.postDelayed({
                ...
                editorStack.push(mergeTextChanges)
                // clear the merge list
                mergeTextChanges = null
                // now we can undo
                onMergedAfter()
    ...
    open fun undo(): Boolean {
        ...
        return editorStack.canUndo()
    }

    open fun redo(): Boolean {
        val changes = editorStack.redo()
        val operations = mutableListOf<SingleEditOperation>()
        changes?.forEach {
    ...
    fun replace(
        replacement: String, 
        matches: MutableList<Range> = searchMatches
    ) {
    ...
    protected inner class TextCursor(position: Position?) : Position() {
        ...
        tailrec fun makeBlink(delay: Long, start: Long, interval: Long) {
    ...
    protected fun scaleCallback(
        ...
        val line = textLayout.run {
            ...
            // relayout after
            getIndexAt(result.line, result.start + 1)
        }
        // update...
        updateDrawableBounds()
        updateDisplayList()
