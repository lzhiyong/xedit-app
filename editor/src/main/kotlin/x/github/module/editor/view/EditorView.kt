...
open class EditorView @JvmOverloads constructor(
    ...
    init {
        ...
        vertScrollbarAnimator = ValueAnimator.ofInt(0, vertScrollbarThumb.bounds.width()).apply {
            ...
            addListener(object: AnimatorListenerAdapter() {
                override fun onAnimationEnd(animation: Animator) {
                    removeOverlay(vertScrollbarThumb)
                }
            })
        ...
        horizScrollbarAnimator = ValueAnimator.ofInt(0, horizScrollbarThumb.bounds.height()).apply {
            ...
            addListener(object: AnimatorListenerAdapter() {
                override fun onAnimationEnd(animation: Animator) {
                    removeOverlay(horizScrollbarThumb)
                }
            })
    ...
    override fun verifyDrawable(who: Drawable): Boolean {
        return overlayDrawables.contains(who)
    }
    
    open fun preserveState(uriPath: String, fileHash: String): SavedState {     
        return SavedState(
            uri = uriPath,
            hash = fileHash,
    ...
    open fun restoreState(
        ...
        cancellCallback: () -> Boolean
    ) {        
        with(savedState) {
             scrollTo(0, 0)
             pieceTreeBuffer = textBuffer
             textLayout.relayout(
                 isCancelled = cancellCallback,
                 onCompleted = { recycleRenderNode() }
             )
    ...
    override fun onScrollChanged(l: Int, t: Int, oldl: Int, oldt: Int) {
        ...
        if (Math.abs(t - oldt) >= Math.abs(l - oldl)) {
            ...
            addOverlay(horizScrollbarThumb)
        ...
        reverseEdits?.let { reverses ->
            ...
            */
            ...
            handler.postDelayed({
                ...
                undoManager.push(mergeTextChanges)
    ...
    protected fun Drawable.contains(x: Int, y: Int): Boolean {
        val result = when(this) {
            ...
            vertScrollbarThumb -> {
                x >= bounds.left - bounds.width() && x <= bounds.right &&
        ...
        return result && verifyDrawable(this)
    }
    ...
    protected inner class TextCursor(position: Position? = null) : Position() {
        ...
        fun setCoordinates(x: Int, y: Int) {                       
            // measured line number
            val line = Math.max(y, 0) / getLineHeight() + 1
            val result = textLayout.getLineResult(line)    
        ...
        fun cancelBlink(keepCursor: Boolean, keepHandleMiddle: Boolean) {
            ...
            when(keepCursor) {
                true -> addOverlay(cursorBarThumb)
                else -> removeOverlay(cursorBarThumb)
            }
            
            when(keepHandleMiddle) {
                true -> addOverlay(selectHandleMiddle)
                else -> removeOverlay(selectHandleMiddle)
            }
        }
    ...
    protected inner class TextSelector(range: Range? = null) : Range() {
        ...
        fun select(
            ...
            if(startLine != endLine || startColumn != endColumn) {
                ...
                addOverlay(selectHandleLeft)
                addOverlay(selectHandleRight)
            }
        }
        ...
        fun cancel() {
            removeOverlay(selectHandleLeft)
            removeOverlay(selectHandleRight)
            this.select(1, 1, 1, 1)
        }
    ...
    protected fun moveTo(
        ...
        right: Int = visibleWidth - SCROLL_DELTA,
    ...
    override fun onScroll(e1: MotionEvent?, e2: MotionEvent, distanceX: Float, distanceY: Float): Boolean {
        ...
        if (selectHandleMiddle.isPressed) {
            ...
            val scrolled = moveTo(
                left = SCROLL_DELTA, 
                top = getLineHeight(), 
                right = visibleWidth - paddingHorizontal - SCROLL_DELTA, 
            ...
            val scrolled = moveTo(
                left = SCROLL_DELTA, 
                top = getLineHeight(), 
                right = visibleWidth - paddingHorizontal - SCROLL_DELTA - widthDelta, 
                bottom = visibleHeight - paddingVertical - getLineHeight() * 3
            )
            ...
            if (scroller.currY + distanceY < 0f) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S)                                       
                    ...
                    edgeEffectTop.onPull(-distanceY / getHeight(), 1 - e2.getX() / getWidth())
                edgeEffectTopActive = true
            } else if (scroller.currY + distanceY > maxScrollY.toFloat()) {                                
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S)
                    edgeEffectBottom.onPullDistance(distanceY / getHeight(), e2.getX() / getWidth())
                else
                    edgeEffectBottom.onPull(distanceY / getHeight(), e2.getX() / getWidth())
                edgeEffectBottomActive = true
            }
