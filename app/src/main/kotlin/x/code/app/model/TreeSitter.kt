...
/**
 * The tree-sitter model class
 *
 * @types the suffix json config file, for detect the file types
 * @styles the syntax highlight style json config file
 */
class TreeSitter(private val context: Context) {
     ...
     *     
     * @name the name of document file 
     * @textBuffer contents of the text editor
     * @return
     */
    fun init(name: String, textBuffer: PieceTreeTextBuffer) {
        val language = getLanguage(name)
        // the query files directory /data/data/package_name/files/query
        val queryDir = File(context.getFilesDir(), "query")
        val pattern = language?.run {
            getPattern(queryDir, language.getName(), "highlights") 
        } ?: null        
        // initialize the tree-sitter
        if (language != null && pattern != null) {
            tsParser = TSParser(language)
            tsQuery = TSQuery(language, pattern)
            // first time parse the oldTree is null
            tsTree = parse(null, textBuffer)           
            isEnabled = true
        }
    }
