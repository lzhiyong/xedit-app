...
public class EditorService : Service() {
    ...
    fun getSerializeFile(uri: Uri): File {
        return File(getFilesDir(), "serialize/${getSerializeName(uri)}.cbor")  
    }
    ...
    @WorkerThread
    suspend fun serialize(uri: Uri, editor: EditorView) {
        val fileName = getSerializeName(uri)
        val savedDir = File(getFilesDir(), "serialize").apply {
            if(!exists()) mkdirs()
        }
    ...
    @WorkerThread
    suspend fun deserialize(uri: Uri): SavedState? {
        var savedState: SavedState? = null
        val fileName = getSerializeName(uri)        
        deserializeStream(
            File(getFilesDir(), "serialize/${fileName}.cbor"),
            callback = { bis ->
                @OptIn(kotlinx.serialization.ExperimentalSerializationApi::class)          
