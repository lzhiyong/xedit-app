...
class HighlightTextView @JvmOverloads constructor(
    ...
    override fun undo(): Boolean {
        if (isEditable() && undoManager.canUndo()) {
            super.undo()
            viewModel.setCanUndo(undoManager.canUndo())
            viewModel.setCanRedo(undoManager.canRedo())
        }
        return viewModel.canUndo.value
    }
    
    override fun redo(): Boolean {
        if (isEditable() && undoManager.canRedo()) {
            super.redo()
            viewModel.setCanRedo(undoManager.canRedo())
            viewModel.setCanUndo(undoManager.canUndo())
        }
    ...
    override fun drawText(
        ...
        if (text.length > 0 && treeSitter.isEnabled) {
            ...
            val spannable = treeSitter.queries(text, lineStart, startOffset, endOffset)
    ...
    override fun restoreState(savedState: SavedState, cancellCallback: () -> Boolean) {
        // here request restores the scroll state
        //scrollTo(0, 0)
        // now restore the editor states
        super.restoreState(savedState, cancellCallback)
