...
import x.github.module.treesitter.TSQueryCursor
...
class TreeSitter(private val context: Context) {
    ...
    private lateinit var cursor: TSQueryCursor 
    ...
    fun init(fileName: String, textBuffer: PieceTreeTextBuffer) {      
        // the symbol name `tree_sitter_c`, `tree_sitter_cpp` etc
        val symbol = getSymbol(fileName)
        val libpath = "${context.getFilesDir()}/lib/aarch64/lib${symbol}.so"
        val language = symbol?.run { getLanguage(libpath, this) }
            
        val pattern = if (language != null && symbol != null) {
            // the query files directory, default is /data/data/package_name/files/query
            getPattern(
                File(context.getFilesDir(), "query"), 
                language.name ?: symbol.substring(12).lowercase(), 
                "highlights"
            )
        } else null        
        ...
        if (language != null && pattern != null) {
            this.parser = TSParser(language)
            this.query = TSQuery(language, pattern)
            // first time parse the oldTree is null
            this.tree = parse(null, textBuffer)           
     ...
     * for the same file, oldTree is equivalent to the current tree
     *
     * @textBuffer the piece tree text buffer
     * @return the new TSTree
     */
    fun parse(textBuffer: PieceTreeTextBuffer) = parse(tree, textBuffer)
    ...
    fun parse(oldTree: TSTree?, textBuffer: PieceTreeTextBuffer): TSTree {        
        ...
        if(textBuffer.length > 2097152) {
            ...
            // parse callback
            tree = parser.parse(oldTree) { byteIndex, point ->            
            ...
            tree = parser.parse(oldTree, textBuffer.toString())
        }
        // return the new TSTree
        return tree
    }
     ...
     */
    fun queries(text: String, lineStart: Int, startOffset: Int, endOffset: Int): SpannableString {
            ...
            for (capture in match.captures) {
                ...
                spanTypeMap[capture.name]?.let { span ->              
                    ...
                    if (spanMark != null && start >= markEnd) {                        
                        val spanArray = spannable.getSpans(markStart, markEnd, CharacterStyle::class.java).apply {
                            sortBy { spannable.getSpanStart(it) }
                        }
                        
                        var start = spannable.getSpanStart(spanArray[0])
                        var end = spannable.getSpanEnd(spanArray[0])                                               
                        if (start != markStart) {                            
                            spannable.setSpan(
                        ...
                        for (i in 1..spanArray.size - 1) {
                            start = spannable.getSpanStart(spanArray[i])
                            ...
                            end = spannable.getSpanEnd(spanArray[i])
                        }
     ...
     * @return TSLanguage
     */
    fun getLanguage(path: String?, symbol: String?): TSLanguage? {
        return if (path != null && symbol != null) TSLanguage(path, symbol) else null
    }
    
    fun getSymbol(filename: String): String? {
        val index = filename.lastIndexOf(".")
        return if (index >= 0) {
            // file extension name
            val extension = filename.substring(index)
            // get the grammar symbol name via the file extension name
            // like `tree_sitter_c`, `tree_sitter_cpp` etc
            fileTypeMap[extension]           
        } else {
            // get the grammar symbol name via the file full name
            fileTypeMap[filename]
        }
    }
