...
data class TextChange(
    ...
    override fun toString(): String {
        if(this.oldText.length == 0) {
            return "TextChange(insert@${this.oldPosition} ${Strings.escapeLineFeed(this.newText)})"
        }
        if(this.newText.length == 0) {
            return "TextChange(delete@${this.oldPosition} ${Strings.escapeLineFeed(this.oldText)})"
        }
        return "TextChange(replace@${this.oldPosition} ${Strings.escapeLineFeed(this.oldText)} -> ${Strings.escapeLineFeed(this.newText)})"
    }
}
