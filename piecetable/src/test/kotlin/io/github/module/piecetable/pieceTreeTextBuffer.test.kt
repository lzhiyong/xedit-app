...
class PieceTreeTextBufferTest {
    ...
    @Test
    fun `getNearestChunk`() {
        val pieceTree = createTextBuffer(["012345678"])
    ...
    fun `bug 45892 For some cases the buffer is empty but we still try to search`() {
        ...
        val ret = pieceTree.findMatchesLineByLine(Range(1, 1, 1, 1), SearchData("abc".toRegex(), ",./", "abc"), 1000)
    ...
    fun `bug 45770 FindInNode should not cross node boundary`() {
        ...
        pieceTree.insert(16, " ")
        val ret = pieceTree.findMatchesLineByLine(Range(1, 1, 4, 13), SearchData("\\[".toRegex(), ",./", null), 1000)
    ...
    fun `search searching from the middle`() {
        ...
        var ret = pieceTree.findMatchesLineByLine(Range(2, 3, 2, 6), SearchData("a".toRegex(), null, "a"), 1000)
        assertEquals(ret.size, 1)
        assertEquals(ret[0], Range(2, 3, 2, 4))

        pieceTree.delete(4, 1)
        ret = pieceTree.findMatchesLineByLine(Range(2, 2, 2, 5), SearchData("a".toRegex(), null, "a"), 1000)
        assertEquals(ret.size, 1)
